 ...
 *     sitekey: "0x4AAAAAAChnKAZBY0iFpFHC",
 *     pageurl: "https://exemplo.com/pagina",
 *     action:  "signup_email_password",  // opcional
 *     chaveApi: "sua-chave-nopecha",     // opcional — usa NOPECHA_API_KEY do env
 *   });
 *
 *   if (resultado.status) {
...
const NopechaService = {
  ...
  resolverTurnstile: async ({ sitekey, pageurl, action, chaveApi } = {}) => {
    const chave = chaveApi || process.env.NOPECHA_API_KEY || '';

    if (!chave) {
      const msg = 'NOPECHA_API_KEY não configurada e chaveApi não fornecida';
      console.error(`[NOPECHA-TURNSTILE] ${msg}`);
      return { status: false, msg };
    }

    if (!sitekey || !pageurl) {
      const msg = 'Parâmetros obrigatórios ausentes: sitekey e pageurl são necessários';
      console.error(`[NOPECHA-TURNSTILE] ${msg}`);
      return { status: false, msg };
    }
    ...
    try {
      ...
      console.log(`[NOPECHA-TURNSTILE] Submetendo job... sitekey=${sitekey.substring(0, 20)} | pageurl=${pageurl}`);
