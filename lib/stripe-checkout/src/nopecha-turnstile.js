...
const ACTION   = "signup_email_password";
const PROXY    = "gw.dataimpulse.com:19991:ee33ae60999c46254f38__cr.br:3d598f73165017aa";
// ─────────────────────────────────────────────────────────────────────────────
...
const NopechaService = {
   ...
   */
  resolverTurnstile: async ({ sitekey, pageurl, action, proxy, chaveApi } = {}) => {
    ...
    try {
      ...
      // Converte proxy "host:port:user:pass" → objeto { scheme, host, port, username, password }
      // Usa limit 4 para que senhas com ":" não sejam quebradas
      const [proxyHost, proxyPort, proxyUser, ...proxyPassParts] = _proxy.split(':');
      const proxyPass = proxyPassParts.join(':');
      const proxyObjeto = {
        scheme: 'http',
        host: proxyHost,
      ...
      if (_action) corpo.data = { action: _action };

      console.log(`[NOPECHA-TURNSTILE] Payload: ${JSON.stringify(corpo)}`);
