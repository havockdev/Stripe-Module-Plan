 ...
 *   - status: 'paid' | 'cartao_recusado' | 'erro' | 'erro_fatal'
 ...
 *   }
 * })
 * console.log(resultado.status) // 'paid' ou 'cartao_recusado'
 */
export async function processarPagamento(link, opcoes = {}) {
  ...
  try {
    ...
    const idMetodoPagamento = await automacao.criarMetodoPagamento(cartao, endereco);
    resultado = await automacao.confirmarPagamento(idMetodoPagamento);
  } catch (erro) {
    registrar(`Erro fatal na execução: ${erro.message}`, 'error');
    resultado = { sucesso: false, status: 'erro_fatal', mensagem: erro.message };
  } finally {
