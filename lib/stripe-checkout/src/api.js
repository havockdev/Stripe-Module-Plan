 ...
 * @param {string} [opcoes.proxy=''] - Proxy residencial no formato "host:porta:usuario:senha".
 *   Passe string vazia ou omita para conexão direta (sem proxy).
 *   Exemplo: "geo.iproyal.com:12321:usuario:senha_country-br_session-xxx"
...
export async function processarPagamento(link, opcoes = {}) {
  const { codigos = [], proxy = '', cartao, endereco } = opcoes;

  console.log(`\n${'='.repeat(100)}`);
  console.log(` INICIANDO AUTOMAÇÃO DE PAGAMENTO STRIPE`);
  console.log(`${'='.repeat(100)}`);
  console.log(` URL: ${link}`);
  if (proxy) console.log(` PROXY: ${proxy.split(':').slice(0, 2).join(':')}`);

  const automacao = new AutomacaoCheckout(link, proxy);
  let resultado;
