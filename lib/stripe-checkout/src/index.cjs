...
class AutomacaoCheckout {
  ...
  /**
   * Preenche o formulário de cartão no browser e confirma via Stripe.js nativo (hCaptcha incluso).
   *
  ...
  async preencherCartaoEConfirmar(dadosCartao, dadosEndereco) {
    return (await this._obterInstancia()).preencherCartaoEConfirmar(dadosCartao, dadosEndereco);
  }
