  ...
  async preencherCartaoEConfirmar(dadosCartao, dadosEndereco) {
    ...
    if (respostaConfirm) {
      ...
      if (statusSessao === 'open') {
        // Detecta 3DS imediatamente — não aguarda redirect inútil
        const requer3DS =
          respostaConfirm.setup_intent?.status === 'requires_action' &&
          respostaConfirm.setup_intent?.next_action?.type === 'use_stripe_sdk';
        if (requer3DS) {
          registrar(
            '[3DS] Autenticação bancária obrigatória (SCA/3DS) — Stripe exige verificação do banco para esta conta. A automação não consegue resolver.',
            'warning'
          );
          return {
            sucesso: false,
        ...
        // Para assinaturas (total 0), o Stripe pode manter 'open' no confirm
