    ...
    if (respostaConfirm) {
      ...
      if (statusSessao === 'open') {
        ...
        if (requerAcaoSCA) {
          ...
          // ── intent_confirmation_challenge: Stripe exige hCaptcha anti-fraude ──────
          // Esta é a causa real dos erros. Não é 3DS bancário.
          // O Stripe carrega um segundo hCaptcha visível via verification_url antes de confirmar o setup_intent.
          // Como não há interação humana, o captcha nunca é resolvido.
          // Comportamento: falhar rápido (sem esperar 65s) e retornar erro de captcha.
          if (tipoNextAction === 'intent_confirmation_challenge') {
            const verifyUrl = si?.next_action?.use_stripe_sdk?.stripe_js?.verification_url || '';
            registrar(
              `[CAPTCHA] Stripe exige hCaptcha anti-fraude (intent_confirmation_challenge). verification_url: ${verifyUrl}`,
              'warning'
            );

            // Aguarda 5s para o captcha renderizar completamente
            registrar('[CAPTCHA] Aguardando 5s para renderizar a tela do captcha...', 'info');
            await this.pagina.waitForTimeout(5000);
            ...
            try {
              ...
              const nomePrint = path.join(dirPrints, `captcha_${timestamp}.png`);
              await this.pagina.screenshot({ path: nomePrint, fullPage: true });
              registrar(`[CAPTCHA] Print salvo em: ${nomePrint}`, 'success');
            } catch (errPrint) {
              registrar(`[CAPTCHA] Falha ao salvar print: ${errPrint.message}`, 'warning');
            }

            return {
              sucesso: false,
              status: 'error',
              mensagem: 'Stripe exige verificação de captcha anti-fraude para esta sessão — não é possível confirmar automaticamente',
            };
          }
