  ...
  async preencherCartaoEConfirmar(dadosCartao, dadosEndereco) {
    ...
    if (respostaConfirm) {
      ...
      if (statusSessao === 'open') {
        ...
        const requerAcaoSCA =
          respostaConfirm.setup_intent?.status === 'requires_action' &&
          respostaConfirm.setup_intent?.next_action?.type === 'use_stripe_sdk';

        if (requerAcaoSCA) {
          registrar(
            '[SCA] setup_intent requires_action/use_stripe_sdk — aguardando Stripe.js completar ação (pode ser SCA silencioso)...',
            'info'
          );
          ...
          // Redirecionamento → SCA silencioso completado com sucesso
          ...
          if (segundaRespHttp) {
            ...
            if (segundoConfirm) {
              ...
              const s2Pagamento = segundoConfirm.payment_status;
              registrar(`[SCA] Segundo confirm: status=${s2Status} payment_status=${s2Pagamento}`, 'info');

              if (s2Status === 'complete') {
                registrar('Pagamento confirmado após SCA silencioso.', 'success');
                return { sucesso: true, status: 'paid', mensagem: 'Pagamento confirmado (SCA silencioso)' };
              }
              ...
              if (s2Status === 'open') {
                // Pode ser assinatura que ainda demora → aguarda redirect
                const redirectPosSCA = await this.pagina.waitForURL(urlSucesso, { timeout: 30000 }).then(() => true).catch(() => false);
                if (redirectPosSCA) {
                  registrar('Redirecionamento tardio após segundo confirm SCA — pagamento concluído.', 'success');
          ...
          // Nenhum segundo confirm nem redirect → banco exige 3DS interativo real
          registrar(
            '[3DS] Autenticação bancária interativa necessária — banco exige verificação manual. Automação não consegue resolver.',
            'warning'
          );
          return {
            sucesso: false,
