...
export class AutomacaoCheckout {
  ...
  async extrairDadosSessao() {
    ...
    // Se o interceptor de requisições não capturou os IDs de rastreamento,
    // gera UUIDs aleatórios — o Stripe aceita ambos os casos
    ...
    registrar(
      `IDs de rastreamento: GUID=${this.dadosStripe.guid.substring(0, 8)}..., MUID=${this.dadosStripe.muid.substring(0, 8)}...`,
      'info',
    );
  }
  ...
  async confirmarPagamento(idMetodoPagamento) {
    ...
    parametros.append('expected_amount', '0');
    ...
    try {
      ...
      if (dados.error) {
        ...
        return { sucesso: false, status: 'erro', mensagem: dados.error.message };
      }

      registrar('Pagamento confirmado com sucesso!', 'success');
      registrar(`Status: ${dados.status || 'N/A'}`, 'success');
      return {
        sucesso: true,
        status: dados.status,
      ...
      return { sucesso: false, status: 'erro', mensagem: erro.message };
    }
  }
  ...
  async executar(opcoes = {}) {
    ...
    try {
      ...
      return { sucesso: false, status: 'erro_fatal', mensagem: erro.message };
    } finally {
      await this.fechar();
    }
  }
