...
async function resolverCaptchaEzcaptcha({ siteKey, rqdata, websiteUrl }) {
  ...
  // Usa proxy da VPS se configurado, senão tenta direto (pode não funcionar no Replit)
  const baseUrl = (process.env.EZCAPTCHA_PROXY_URL || 'https://api.ezcaptcha.com').replace(/\/$/, '');
  registrar(`[EZCAPTCHA] Base URL: ${baseUrl}`, 'info');
  ...
  // Fetch com timeout de 30s via AbortController
  const chamarEzcaptcha = async (caminho, payload) => {
    ...
    try {
      ...
      if (err.name === 'AbortError') throw new Error(`Timeout de 30s ao chamar ${baseUrl}${caminho}`);
      throw err;
    } finally {
      clearTimeout(timer);
    }
  ...
  const dadosCriar = await chamarEzcaptcha('/createTask', corpo);
  ...
  for (let tentativa = 0; tentativa < 40; tentativa++) {
    ...
    registrar(`[EZCAPTCHA] Polling tentativa ${tentativa + 1}/40...`, 'info');
    const dadosResultado = await chamarEzcaptcha('/getTaskResult', { clientKey: chaveApi, taskId });
    ...
    registrar(`[EZCAPTCHA] Status: ${dadosResultado.status} — aguardando... (${(tentativa + 1) * 3}s)`, 'info');
  }
