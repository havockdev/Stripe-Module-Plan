...
export class AutomacaoCheckout {
  ...
  constructor(urlCheckout) {
    ...
    this.dadosStripe = {
      ...
      chavePublica: null,
      /** Identificador de rastreamento do dispositivo — capturado do browser ou gerado aleatoriamente */
      guid: null,
      /**
       * Identificador de rastreamento do navegador (machine unique ID).
       * Inicializado com um valor real capturado de sessão Stripe legítima.
       * Pode ser sobrescrito pelo interceptor de requisições ou pelo cookie __stripe_mid.
       * O Stripe reconhece este valor como dispositivo confiável.
       */
      muid: '24F6C9D492896DCB0398DF62939D6C4A',
      /** Identificador de rastreamento da sessão — capturado do cookie __stripe_sid */
      sid: null,
      /** Cookies da sessão do browser, usados no header Cookie da confirmação */
      cookies: [],
      /**
       * User-Agent do Firefox para iOS (iPhone).
       * Idêntico ao do módulo de referência — melhora a taxa de aceite pelo Stripe.
       */
      agenteUsuario:
        'Mozilla/5.0 (iPhone; CPU iPhone OS 17_4_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) FxiOS/144.0 Mobile/15E148 Safari/605.1.15',
  ...
  async extrairDadosSessao() {
    ...
    // Gera guid e sid aleatórios apenas se ainda não foram capturados
