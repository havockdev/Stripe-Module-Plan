...
export class AutomacaoCheckout {
  ...
  constructor(urlCheckout, proxy = '') {
    ...
    this.proxy = null;
    if (proxy && proxy.trim()) {
      // Formato: host:porta:usuario:senha (senha pode conter ':')
      const partes = proxy.trim().split(':');
      const host = partes[0];
      const porta = partes[1];
      const usuario = partes[2];
      const senha = partes.slice(3).join(':');
      this.proxy = { server: `http://${host}:${porta}`, username: usuario, password: senha };
      registrar(`Proxy configurado: ${host}:${porta} (usuário: ${usuario})`, 'info');
    }
  ...
  async inicializar() {
    ...
    // Adiciona proxy ao contexto se configurado
    if (this.proxy) {
