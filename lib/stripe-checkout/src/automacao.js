...
async function resolverCaptchaEzcaptcha({ siteKey, rqdata, websiteUrl }) {
  ...
  // Usa proxy da VPS se configurado, senão chama api.ez-captcha.com diretamente
  const baseUrl = (process.env.EZCAPTCHA_PROXY_URL || 'https://api.ez-captcha.com').replace(/\/$/, '');
