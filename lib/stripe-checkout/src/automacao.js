      ...
      if (statusSessao === 'open') {
        ...
        if (requerAcaoSCA) {
          ...
          if (tipoNextAction === 'intent_confirmation_challenge') {
            ...
            if (!sitekey && frameHcaptcha) {
              try {
                // URL do iframe: https://newassets.hcaptcha.com/.../hcaptcha.html#sitekey=xxx&...
                // ou query: ?sitekey=xxx
                const iframeUrl = frameHcaptcha.url();
                const urlObj = new URL(iframeUrl);
                sitekey = urlObj.searchParams.get('sitekey') ||
            ...
            if (tokenNopecha) {
              ...
              try {
                await this.pagina.evaluate((token) => {
                  ...
                  // (d) Despacha custom event para que o Stripe.js (ou qualquer listener) possa reagir
                  try {
