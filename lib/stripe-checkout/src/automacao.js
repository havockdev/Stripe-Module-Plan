 ...
 *
 * Usa Playwright (Chromium + stealth + display virtual) para:
 ...
 *
 * O stealth plugin (puppeteer-extra-plugin-stealth) aplica ~20 patches via CDP
 * no Chromium para tornar o browser indistinguível de um usuário real,
 * impedindo que o hCaptcha eleve o challenge para puzzle visual.
 */
...
/**
 * Encontra o executável do Chromium do sistema (instalado via Nix no Replit).
 * O Playwright baixa seu próprio Chromium, mas no NixOS as bibliotecas do sistema
 * estão em locais não-padrão, fazendo o binário baixado falhar.
 * O Chromium do Nix já vem com todas as dependências resolvidas.
 *
 * @returns {string|undefined} Caminho para o chromium ou undefined para usar o padrão do Playwright
 */
function encontrarChromiumSistema() {
  try {
    const caminho = execSync('which chromium 2>/dev/null || which chromium-browser 2>/dev/null || which google-chrome 2>/dev/null', { encoding: 'utf8' }).trim().split('\n')[0];
    if (caminho) {
      registrar(`Chromium do sistema encontrado: ${caminho}`, 'info');
      return caminho;
    }
  } catch (_ignorado) {}
  return undefined;
...
// Registra o stealth plugin globalmente — aplica patches CDP antes de qualquer launch()
...
export class AutomacaoCheckout {
  ...
  constructor(urlCheckout, proxy = '') {
    ...
    this.dadosStripe = {
      ...
      cookies: [],
      /** User-Agent do Chrome simulado nas requisições HTTP diretas */
      agenteUsuario:
  ...
  /**
   * Inicializa o navegador Chromium com stealth (puppeteer-extra-plugin-stealth).
   *
   * O stealth plugin aplica ~20 patches via CDP antes do launch(), tornando o
   * Chromium indistinguível de um usuário real para o hCaptcha:
   * - Remove navigator.webdriver
   * - Corrige user-agent (remove "HeadlessChrome")
   * - Emula plugins, WebGL, canvas fingerprint, etc.
   *
   ...
   * (variável de ambiente DISPLAY, ex: ":0").
   *
   * @returns {Promise<void>}
   * @throws {Error} Se o Playwright não conseguir lançar o Chromium
   */
  async inicializar() {
    registrar('Inicializando navegador Chromium (stealth + display virtual)...', 'info');

    const executablePath = encontrarChromiumSistema();
    ...
    // Chromium suporta SOCKS5 autenticado nativamente — passa proxy direto sem túnel
    if (this.proxy) {
      opcoesContexto.proxy = this.proxy;
      registrar(`Contexto Playwright criado com proxy: ${this.proxy.server}`, 'info');
    }
  ...
  async preencherCartaoEConfirmar(dadosCartao, dadosEndereco) {
    ...
    // ── Botão de pagamento ────────────────────────────────────────────────────
    // ── Desmarcar "Save my information for faster checkout" (Stripe Link) ──────
    // Se este checkbox estiver marcado, o Stripe exige verificação de telefone via SMS (OTP),
    // bloqueando o pagamento silenciosamente (sem chamar /confirm).
    // Precisamos desmarcá-lo antes de clicar em Subscribe.
    try {
      const checkboxLink = await localizarEmFrames(async (frame) => {
        // Tenta vários seletores conhecidos do Stripe Link
        return await frame.$(
          'input[data-testid="link-toggle"], ' +
          'input[data-testid="link-opt-in"], ' +
          'input[name="enableStripePass"], ' +
          'input[type="checkbox"][data-testid*="link"], ' +
          'input[type="checkbox"][data-testid*="save"]'
        );
      });
      if (checkboxLink) {
        const marcado = await checkboxLink.elemento.isChecked();
        if (marcado) {
          await checkboxLink.elemento.click();
          registrar('[STRIPE LINK] Checkbox "Save my information" desmarcado.', 'info');
          await this.pagina.waitForTimeout(500);
        } else {
          registrar('[STRIPE LINK] Checkbox "Save my information" já estava desmarcado.', 'info');
        }
      } else {
        // Tenta via evaluate diretamente na página principal também
        const desmarcado = await this.pagina.evaluate(() => {
          const seletores = [
            'input[data-testid="link-toggle"]',
            'input[data-testid="link-opt-in"]',
            'input[name="enableStripePass"]',
        ...
        if (desmarcado) {
          registrar('[STRIPE LINK] Checkbox desmarcado via evaluate na página principal.', 'info');
          await this.pagina.waitForTimeout(500);
        } else {
          registrar('[STRIPE LINK] Nenhum checkbox de Stripe Link encontrado (pode já estar inativo).', 'info');
        }
      }
    } catch (errLink) {
      registrar(`[STRIPE LINK] Erro ao tentar desmarcar checkbox: ${errLink.message}`, 'warning');
    }
    ...
    // Screenshot diagnóstico — estado do formulário imediatamente ANTES do clique
    try {
      const _dirPrints = path.resolve('screenshots');
      if (!fs.existsSync(_dirPrints)) fs.mkdirSync(_dirPrints, { recursive: true });
      const tsAntes = new Date().toISOString().replace(/[:.]/g, '-');
      await this.pagina.screenshot({ path: path.join(_dirPrints, `antes_clique_${tsAntes}.png`), fullPage: true });
      registrar(`[DIAG] Screenshot antes_clique salvo`, 'info');
    } catch (_e) {}

    await botaoPagar.elemento.click();
    registrar('Botão de pagamento clicado — aguardando Stripe.js processar (hCaptcha + confirm)...', 'info');

    // Listener diagnóstico: loga TODAS as respostas Stripe após o clique
    // Ajuda a entender o que acontece na rede (hCaptcha, confirm, errors, etc.)
    const ouvinteDiagnostico = async (resp) => {
      const url = resp.url();
      if (
        url.includes('api.stripe.com') ||
        url.includes('hcaptcha.com') ||
        url.includes('m.stripe.com') ||
        url.includes('r.stripe.com')
      ) {
        const status = resp.status();
        const urlCurta = url.length > 120 ? url.substring(0, 120) + '...' : url;
        registrar(`[NET] ${status} ${urlCurta}`, 'info');
      }
    };
    this.pagina.on('response', ouvinteDiagnostico);
    ...
    // Remove o listener diagnóstico após o wait
    this.pagina.off('response', ouvinteDiagnostico);
    ...
    if (respostaConfirm) {
      ...
      if (statusSessao === 'open') {
        // setup_intent OU payment_intent requires_action/use_stripe_sdk:
        ...
        // ou redirecionar para success_url. Só reportar erro 3DS se não acontecer nada em 65s.
        const si = respostaConfirm.setup_intent;
        const pi = respostaConfirm.payment_intent;
        const requerAcaoSCA =
          (si?.status === 'requires_action' && si?.next_action?.type === 'use_stripe_sdk') ||
          (pi?.status === 'requires_action' && pi?.next_action?.type === 'use_stripe_sdk');

        // Para o diagnóstico: qual intent está em requires_action
        const intentAtivo = (si?.status === 'requires_action') ? 'setup_intent' : 'payment_intent';
        const intentObj = (si?.status === 'requires_action') ? si : pi;

        if (requerAcaoSCA) {
          const tipoNextAction = intentObj?.next_action?.use_stripe_sdk?.type || 'desconhecido';
          registrar(
            `[SCA] ${intentAtivo} requires_action | subtipo: ${tipoNextAction} | id: ${intentObj?.id?.substring(0, 30)}`,
            'info'
          );

          // ── intent_confirmation_challenge: hCaptcha checkbox "Sou humano" ─────────
          // O Stripe abre um modal com checkbox hCaptcha antes de confirmar o setup_intent.
          // Solução: localizar o iframe do hCaptcha, clicar #checkbox e aguardar segundo /confirm.
          if (tipoNextAction === 'intent_confirmation_challenge') {
            const verifyUrl = intentObj?.next_action?.use_stripe_sdk?.stripe_js?.verification_url || '';
            registrar(
              `[CAPTCHA] intent_confirmation_challenge detectado. Tentando clicar checkbox hCaptcha... verification_url: ${verifyUrl}`,
              'info'
            );

            // Registra listeners ANTES de interagir (sem race condition)
            ...
            const promessaRedirectCaptcha = this.pagina
              .waitForURL(urlSucesso, { timeout: 45000 })
              .then(() => true)
              .catch(() => false);

            // Busca o frame hcaptcha.html que tem #checkbox VISÍVEL agora.
            // Múltiplos frames podem ter a mesma URL (mesma versão estática do hCaptcha),
            // então filtramos pelo estado real do DOM — só o challenge ativo tem #checkbox visível.
            ...
            for (let tentativa = 0; tentativa < 40; tentativa++) {
              ...
              for (const f of frames) {
                try {
                  // Verifica se #checkbox existe e está visível neste frame específico
            ...
            if (!frameHcaptcha) {
              registrar('[CAPTCHA] iframe NOVO do hCaptcha não encontrado após 20s', 'warning');
              const todosFrames = this.pagina.frames().map(f => f.url()).filter(u => u && u !== 'about:blank');
              registrar(`[CAPTCHA] Todos os frames ativos: ${JSON.stringify(todosFrames.slice(0, 15))}`, 'info');
            } else {
              registrar(`[CAPTCHA] iframe hCaptcha NOVO encontrado: ${frameHcaptcha.url().substring(0, 80)}`, 'success');

              // Tira print ANTES do clique
              try {
                const dirPrints = path.resolve('screenshots');
                if (!fs.existsSync(dirPrints)) fs.mkdirSync(dirPrints, { recursive: true });
                const tsAntes = new Date().toISOString().replace(/[:.]/g, '-');
                const printAntes = path.join(dirPrints, `captcha_antes_${tsAntes}.png`);
                await this.pagina.screenshot({ path: printAntes, fullPage: true });
                registrar(`[CAPTCHA] Print antes do clique: ${printAntes}`, 'info');
              } catch (_) {}

              try {
                // Aguarda o #checkbox ser visível dentro do iframe (até 15s)
                const checkbox = frameHcaptcha.locator('#checkbox');
                await checkbox.waitFor({ state: 'visible', timeout: 15000 });
                registrar('[CAPTCHA] Checkbox #checkbox visível — clicando...', 'info');
                // Pequena pausa para o widget estabilizar antes do clique
                await this.pagina.waitForTimeout(800);
                await checkbox.click();
                registrar('[CAPTCHA] Checkbox clicado! Aguardando hCaptcha verificar...', 'success');
              } catch (errClick) {
                registrar(`[CAPTCHA] #checkbox não ficou visível em 15s: ${errClick.message}`, 'warning');
                // Log do DOM do iframe para diagnóstico
                try {
              ...
              // Tira print DEPOIS do clique (3s depois para ver resultado)
              await this.pagina.waitForTimeout(3000);
              try {
                const dirPrints = path.resolve('screenshots');
                const tsDepois = new Date().toISOString().replace(/[:.]/g, '-');
                const printDepois = path.join(dirPrints, `captcha_depois_${tsDepois}.png`);
                await this.pagina.screenshot({ path: printDepois, fullPage: true });
                registrar(`[CAPTCHA] Print depois do clique: ${printDepois}`, 'info');
              } catch (_) {}
            }

            // Aguarda segundo /confirm OU redirect (até 45s no total)
            registrar('[CAPTCHA] Aguardando Stripe processar após captcha (até 45s)...', 'info');
    ...
    // Screenshot diagnóstico — estado da página no momento do timeout
    try {
      const _dirPrintsT = path.resolve('screenshots');
      if (!fs.existsSync(_dirPrintsT)) fs.mkdirSync(_dirPrintsT, { recursive: true });
      const tsTimeout = new Date().toISOString().replace(/[:.]/g, '-');
      await this.pagina.screenshot({ path: path.join(_dirPrintsT, `timeout_sem_confirm_${tsTimeout}.png`), fullPage: true });
      registrar(`[DIAG] Screenshot timeout_sem_confirm salvo`, 'info');
    } catch (_e) {}
   ...
   *
   * Ordem de execução:
   * 1. inicializar() — lança o Chromium com stealth
