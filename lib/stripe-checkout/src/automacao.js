      ...
      if (statusSessao === 'open') {
        ...
        if (requerAcaoSCA) {
          ...
          if (tipoNextAction === 'intent_confirmation_challenge') {
            ...
            // ── 4a. Injeta o token NopeCHA na página ─────────────────────────────────
            if (tokenNopecha) {
              registrar('[CAPTCHA] Injetando token NopeCHA na página...', 'info');
              try {
                await this.pagina.evaluate((token) => {
                  // (a) Seta todas as textareas h-captcha-response (Stripe.js lê daqui)
                  ...
                  // (b) Chama window.hcaptcha.setResponse(widgetId, token) para cada widget
                  let setResponseDisponivel = false;
                  if (window.hcaptcha && typeof window.hcaptcha.setResponse === 'function') {
                    setResponseDisponivel = true;
                    try {
                  ...
                  // (c) Tenta callback pelo atributo data-callback no container
                  try {
                  ...
                  // (d) Despacha custom event apenas quando setResponse não estava disponível
                  if (!setResponseDisponivel) {
                    try {
                      window.dispatchEvent(new CustomEvent('hcaptcha-response', { detail: { token }, bubbles: true }));
                    } catch (_) {}
                  }
                }, tokenNopecha);

                registrar('[CAPTCHA] Token NopeCHA injetado. Aguardando Stripe.js processar...', 'success');
                ...
                registrar(`[CAPTCHA] Erro ao injetar token: ${errInject.message}`, 'warning');
              }
            } else {
