...
export class AutomacaoCheckout {
  ...
  async capturarValorSessao(acao, timeout = 10000) {
    ...
    try {
      ...
      if (typeof dados.amount_total === 'number') {
        this.dadosStripe.valorEsperado = dados.amount_total;
        registrar(
          `Valor real da sessão capturado: ${dados.amount_total} centavos (amount_total)`,
          'success',
        );
      }
    } catch (_ignorado) {
      // Não bloqueia o fluxo se a resposta não chegar dentro do timeout ou não tiver amount_total
      registrar('Não foi possível capturar amount_total desta resposta — usando valor atual.', 'info');
    }
  }
