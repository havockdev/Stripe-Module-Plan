...
// Singleton do Xvfb — inicia uma vez e reutiliza em todas as requisições
let xvfbIniciado = false;
let xvfbPromise = null;

async function garantirDisplay() {
  if (process.env.DISPLAY) return;
  if (xvfbIniciado) return;
  if (xvfbPromise) return xvfbPromise;

  xvfbPromise = new Promise((resolve, reject) => {
    registrar('Iniciando display virtual Xvfb em :99...', 'info');
    const proc = spawn('Xvfb', [':99', '-screen', '0', '1280x720x24', '-ac', '+extension', 'GLX'], {
      detached: true,
      stdio: 'ignore',
    });
    proc.unref();
    proc.on('error', (err) => {
      registrar(`Falha ao iniciar Xvfb: ${err.message}`, 'error');
      reject(err);
    });
    // Aguarda 1.5s para o Xvfb subir antes de prosseguir
    setTimeout(() => {
      process.env.DISPLAY = ':99';
      xvfbIniciado = true;
      registrar('Display virtual :99 pronto.', 'info');
      resolve();
    }, 1500);
  });
...
export class AutomacaoCheckout {
  ...
  async preencherCartaoEConfirmar(dadosCartao, dadosEndereco) {
    ...
    // Ordem crítica:
    // 1. billingName
    // 2. billingAddressLine1 + Tab → revela campos ocultos (billingLocality, billingAdministrativeArea, billingPostalCode)
    // 3. billingAdministrativeArea (SELECT de estado) — obrigatório para o Brasil
    // 4. billingLocality (cidade)
    // 5. billingPostalCode (CEP)
    ...
    if (respostaConfirm) {
      ...
      if (statusSessao === 'open') {
        // Para assinaturas (total 0 ou com 3DS), o Stripe pode manter 'open' no confirm
        // e redirecionar de forma assíncrona — verifica se o redirect já aconteceu.
