      ...
      if (statusSessao === 'open') {
        ...
        if (requerAcaoSCA) {
          ...
          if (tipoNextAction === 'intent_confirmation_challenge') {
            ...
            // Aguarda o iframe do hCaptcha aparecer (até 12s)
            let frameHcaptcha = null;
            for (let tentativa = 0; tentativa < 24; tentativa++) {
              frameHcaptcha = this.pagina.frames().find(
                (f) =>
            ...
            if (!frameHcaptcha) {
              registrar('[CAPTCHA] iframe do hCaptcha não encontrado após 12s — tirando print diagnóstico', 'warning');
              // Lista todos os frames para diagnóstico
              ...
              try {
                // Aguarda o #checkbox ser visível dentro do iframe
                const checkbox = frameHcaptcha.locator('#checkbox');
                await checkbox.waitFor({ state: 'visible', timeout: 8000 });
                registrar('[CAPTCHA] Checkbox #checkbox encontrado — clicando...', 'info');
                await checkbox.click();
                registrar('[CAPTCHA] Checkbox clicado! Aguardando hCaptcha verificar...', 'success');
              } catch (errClick) {
                // Fallback: tenta clicar pela área central do frame
                registrar(`[CAPTCHA] #checkbox não encontrado (${errClick.message}) — tentando clique por coordenadas`, 'warning');
                try {
                  const viewSize = await frameHcaptcha.evaluate(() => ({
                    w: document.documentElement.scrollWidth,
                    h: document.documentElement.scrollHeight,
                  }));
                  registrar(`[CAPTCHA] Tamanho do iframe: ${JSON.stringify(viewSize)}`, 'info');
                  await frameHcaptcha.locator('body').click({ position: { x: 20, y: 20 }, force: true });
                  registrar('[CAPTCHA] Clique por coordenadas executado', 'info');
                } catch (errFallback) {
                  registrar(`[CAPTCHA] Fallback de clique falhou: ${errFallback.message}`, 'warning');
                }
              }

              // Tira print DEPOIS do clique (2s depois)
              await this.pagina.waitForTimeout(2000);
              try {
                const dirPrints = path.resolve('screenshots');
