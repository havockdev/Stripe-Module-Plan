...
/**
 * Resolve um hCaptcha usando a API de Token da NopeCHA.
 *
 * Submete o sitekey + url + rqdata para a NopeCHA e faz polling até receber
 * o token resolvido (ou retorna null em caso de falha/timeout).
 *
 * @param {{ sitekey: string, url: string, rqdata: string|null, chaveApi: string }} opcoes
 * @returns {Promise<string|null>} Token hCaptcha resolvido, ou null em caso de falha.
 */
async function resolverHcaptchaNopecha({ sitekey, url, rqdata, chaveApi }) {
  try {
    registrar(`[NOPECHA] Submetendo job hCaptcha... sitekey=${sitekey?.substring(0, 20)}`, 'info');

    const corpo = { key: chaveApi, type: 'hcaptcha', sitekey, url };
    if (rqdata) corpo.rqdata = rqdata;

    const respostaSubmit = await fetch('https://api.nopecha.com/v1/token/hcaptcha', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Basic ${chaveApi}`,
      },
    ...
    if (dadosSubmit?.error_id || !dadosSubmit?.data) {
      registrar(`[NOPECHA] Falha ao submeter job: ${JSON.stringify(dadosSubmit)}`, 'warning');
      return null;
    }

    const jobId = dadosSubmit.data;
    registrar(`[NOPECHA] Job submetido com sucesso: ${jobId}. Aguardando resolução (até 90s)...`, 'info');

    // Polling a cada 3s por até 90s (30 tentativas)
    for (let i = 0; i < 30; i++) {
      await new Promise((r) => setTimeout(r, 3000));

      const respostaPoll = await fetch(
        `https://api.nopecha.com/v1/token/hcaptcha?key=${encodeURIComponent(chaveApi)}&id=${encodeURIComponent(jobId)}`,
      );
      const dadosPoll = await respostaPoll.json();

      // error_id 9 = "Incomplete Job" — ainda processando, continuar polling
      if (dadosPoll?.error_id === 9) {
        registrar(`[NOPECHA] Job em processamento... tentativa ${i + 1}/30`, 'info');
        continue;
      }

      if (dadosPoll?.error_id) {
        registrar(`[NOPECHA] Erro no polling: error_id=${dadosPoll.error_id} | ${JSON.stringify(dadosPoll)}`, 'warning');
        return null;
      }

      const token = dadosPoll?.data;
      if (token && typeof token === 'string' && token.length > 20) {
        registrar(`[NOPECHA] Token hCaptcha recebido! (${token.length} chars)`, 'success');
        return token;
      }

      registrar(`[NOPECHA] Resposta inesperada: ${JSON.stringify(dadosPoll).substring(0, 120)}`, 'info');
    }

    registrar('[NOPECHA] Timeout de 90s atingido — token não recebido', 'warning');
    return null;
  } catch (err) {
    registrar(`[NOPECHA] Erro inesperado: ${err.message}`, 'warning');
    return null;
  }
}
...
export class AutomacaoCheckout {
  ...
  constructor(urlCheckout, proxy = '') {
    ...
    this.dadosStripe = {
      ...
      initChecksum: null,
      /**
       * client_secret do setup_intent — capturado da resposta de API do Stripe.js.
       * Necessário para chamar /verify_challenge diretamente com autenticação.
       */
      clientSecretSI: null,
    };
  }
  ...
  async inicializar() {
    ...
    // Relay de console.log do browser → logger do servidor (para diagnóstico de postMessages hCaptcha)
    this.pagina.on('console', (msg) => {
      try {
        const texto = msg.text();
        if (texto.startsWith('[HCAPTCHA-POSTMSG]')) {
          registrar(texto, 'info');
        }
      } catch (_) {}
    });

    // Intercepta respostas da API Stripe para capturar client_secret do setup_intent/payment_intent
    // Necessário para chamar /verify_challenge diretamente com autenticação correta.
    this.pagina.on('response', async (resposta) => {
      try {
        const url = resposta.url();
        if (!url.includes('api.stripe.com') && !url.includes('b.stripecdn.com')) return;
        const ct = resposta.headers()['content-type'] || '';
        if (!ct.includes('application/json')) return;
        const corpo = await resposta.json().catch(() => null);
        if (!corpo) return;
        // Captura client_secret do setup_intent
        if (corpo.client_secret && !this.dadosStripe.clientSecretSI) {
          this.dadosStripe.clientSecretSI = corpo.client_secret;
          registrar(`[SI] client_secret capturado (${String(corpo.client_secret).substring(0, 25)}...) via ${url.substring(0, 80)}`, 'info');
        }
        // Captura via setup_intent aninhado
        if (corpo.setup_intent?.client_secret && !this.dadosStripe.clientSecretSI) {
          this.dadosStripe.clientSecretSI = corpo.setup_intent.client_secret;
          registrar(`[SI] client_secret (aninhado) capturado (${String(corpo.setup_intent.client_secret).substring(0, 25)}...) via ${url.substring(0, 80)}`, 'info');
        }
      } catch (_) {}
    });
  ...
  async preencherCartaoEConfirmar(dadosCartao, dadosEndereco) {
    ...
    // Listener diagnóstico — captura todos os postMessages que chegam dos iframes hCaptcha.
    // Permite identificar o formato exato que o Stripe.js espera.
    await this.pagina.evaluate(() => {
      if (window.__hcaptchaMessageListenerAdded) return;
      window.__hcaptchaMessageListenerAdded = true;
      window.addEventListener('message', (ev) => {
        try {
          const origem = typeof ev.origin === 'string' ? ev.origin : '';
          if (!origem.includes('hcaptcha.com') && !origem.includes('stripe.com') && origem !== '') return;
          const dado = typeof ev.data === 'string' ? ev.data : JSON.stringify(ev.data);
          if (!dado || dado.length < 3) return;
          console.log('[HCAPTCHA-POSTMSG] origem=' + origem + ' | dado=' + dado.substring(0, 300));
        } catch (_) {}
      });
    });
    ...
    if (respostaConfirm) {
      ...
      // Captura client_secret do setup_intent do confirm (se disponível)
      const siDaResposta = respostaConfirm.setup_intent;
      if (siDaResposta?.client_secret && !this.dadosStripe.clientSecretSI) {
        this.dadosStripe.clientSecretSI = siDaResposta.client_secret;
        registrar(`[SI] client_secret capturado do confirm (${siDaResposta.client_secret.substring(0, 25)}...)`, 'info');
      }
      ...
      const camposRelevantes = {
        ...
        setup_client_secret_ok: !!(respostaConfirm.setup_intent?.client_secret),
        setup_chaves: respostaConfirm.setup_intent ? Object.keys(respostaConfirm.setup_intent).join(', ') : null,
      ...
      if (statusSessao === 'open') {
        ...
        if (requerAcaoSCA) {
          ...
          // ── intent_confirmation_challenge: hCaptcha "Sou humano" ──────────────────
          // O Stripe exige que o usuário resolva um hCaptcha antes do confirm final.
          // Estratégia primária: usar a API NopeCHA para obter um token real sem clique.
          // Estratégia fallback: clicar o #checkbox diretamente no iframe.
          if (tipoNextAction === 'intent_confirmation_challenge') {
            const verifyUrl = si?.next_action?.use_stripe_sdk?.stripe_js?.verification_url || '';
            registrar(
              `[CAPTCHA] intent_confirmation_challenge detectado. verification_url: ${verifyUrl}`,
              'info'
            );

            // Extrai client_secret do setup_intent diretamente do confirm response (se disponível)
            if (si?.client_secret && !this.dadosStripe.clientSecretSI) {
              this.dadosStripe.clientSecretSI = si.client_secret;
              registrar(`[SI] client_secret extraído do confirm response (${si.client_secret.substring(0, 25)}...)`, 'info');
            }

            // Tenta extrair client_secret do estado interno do Stripe.js na página
            if (!this.dadosStripe.clientSecretSI) {
              try {
                const csFromPage = await this.pagina.evaluate(() => {
                  // Tenta encontrar client_secret em variáveis globais do Stripe.js
                  let cs = null;
                  try {
                    // Stripe embeds session data in window.__stripe_params__
                    const params = window.__stripe_params__;
                    if (params?.client_secret) { cs = params.client_secret; }
                    if (!cs && params?.setup_intent?.client_secret) { cs = params.setup_intent.client_secret; }
                  } catch (_) {}
                  try {
                    // Checkout embeds JSON data in script tags
                    if (!cs) {
                      const scripts = Array.from(document.querySelectorAll('script:not([src])'));
                      for (const s of scripts) {
                        const match = s.textContent.match(/"client_secret"\s*:\s*"(seti_[^"]+)"/);
                        if (match) { cs = match[1]; break; }
                      }
                    }
                  } catch (_) {}
                  return cs;
                ...
                if (csFromPage) {
                  this.dadosStripe.clientSecretSI = csFromPage;
                  registrar(`[SI] client_secret extraído do estado JS da página (${csFromPage.substring(0, 25)}...)`, 'info');
                } else {
                  registrar('[SI] client_secret NÃO encontrado no estado JS da página', 'warning');
                }
              } catch (errCs) {
                registrar(`[SI] Erro ao buscar client_secret na página: ${errCs.message}`, 'warning');
              }
            }

            // Registra listeners ANTES de qualquer interação (sem race condition)
            ...
            // ── 1. Localiza o iframe do hCaptcha (para extrair sitekey e fallback de clique) ──
            let frameHcaptcha = null;
            registrar('[CAPTCHA] Procurando iframe hCaptcha com #checkbox visível (até 20s)...', 'info');

            for (let tentativa = 0; tentativa < 40; tentativa++) {
              const frames = this.pagina.frames().filter(
                (f) =>
                  f.url().includes('newassets.hcaptcha.com') &&
                  f.url().includes('hcaptcha.html'),
              );
              if (tentativa === 0) {
                registrar(`[CAPTCHA] Total de frames hcaptcha encontrados: ${frames.length}`, 'info');
              }
              for (const f of frames) {
                try {
                  const checkboxVisivel = await f.evaluate(() => {
                    const el = document.querySelector('#checkbox');
                    if (!el) return false;
                    const rect = el.getBoundingClientRect();
                    return rect.width > 0 && rect.height > 0 && el.offsetParent !== null;
                  });
                  if (checkboxVisivel) { frameHcaptcha = f; break; }
                } catch (_) {}
              }
              if (frameHcaptcha) break;
              await this.pagina.waitForTimeout(500);
            }

            if (frameHcaptcha) {
              registrar(`[CAPTCHA] iframe hCaptcha encontrado: ${frameHcaptcha.url().substring(0, 80)}`, 'success');
            } else {
              registrar('[CAPTCHA] iframe hCaptcha não encontrado após 20s', 'warning');
              const todosFrames = this.pagina.frames().map(f => f.url()).filter(u => u && u !== 'about:blank');
              registrar(`[CAPTCHA] Frames ativos: ${JSON.stringify(todosFrames.slice(0, 15))}`, 'info');
            }

            // ── 2. Extrai o sitekey para NopeCHA ───────────────────────────────────────
            // Tenta na resposta do Stripe; depois na URL do iframe; depois no DOM da página.
            let sitekey =
              si?.next_action?.use_stripe_sdk?.stripe_js?.hcaptcha_site_key ||
              si?.next_action?.use_stripe_sdk?.hcaptcha_site_key ||
              null;

            if (!sitekey) {
              // Itera TODOS os frames hcaptcha para extrair sitekey da URL
              // (independente de #checkbox estar visível — o frame pode estar carregado mas não renderizado)
              try {
                const todosFramesHcaptcha = this.pagina.frames().filter(
                  (f) => f.url().includes('newassets.hcaptcha.com') && f.url().includes('hcaptcha.html'),
                );
                for (const f of todosFramesHcaptcha) {
                  const iframeUrl = f.url();
                  const urlObj = new URL(iframeUrl);
                  const sk =
                    urlObj.searchParams.get('sitekey') ||
                    new URLSearchParams(urlObj.hash.replace(/^#/, '')).get('sitekey') ||
                    null;
                  if (sk) { sitekey = sk; break; }
                }
                if (sitekey) registrar(`[CAPTCHA] sitekey extraído da URL do iframe: ${sitekey}`, 'info');
              } catch (_) {}
            }

            if (!sitekey) {
              try {
                sitekey = await this.pagina.evaluate(() => {
                  const el = document.querySelector('[data-sitekey]');
                  return el?.getAttribute('data-sitekey') || null;
                });
                if (sitekey) registrar(`[CAPTCHA] sitekey extraído do DOM: ${sitekey}`, 'info');
              } catch (_) {}
            }

            if (!sitekey) {
              registrar('[CAPTCHA] sitekey não encontrado — NopeCHA não pode ser usado', 'warning');
            } else {
              registrar(`[CAPTCHA] sitekey: ${sitekey}`, 'info');
            }

            // ── 3. Tenta resolver via NopeCHA ─────────────────────────────────────────
            const chaveNopecha = process.env.NOPECHA_API_KEY || '';
            let tokenNopecha = null;

            if (chaveNopecha && sitekey) {
              // Print antes de chamar NopeCHA
              try {
                const dirPrints = path.resolve('screenshots');
                if (!fs.existsSync(dirPrints)) fs.mkdirSync(dirPrints, { recursive: true });
                const ts = new Date().toISOString().replace(/[:.]/g, '-');
                await this.pagina.screenshot({ path: path.join(dirPrints, `captcha_antes_${ts}.png`), fullPage: true });
              } catch (_) {}

              tokenNopecha = await resolverHcaptchaNopecha({
                sitekey,
                url: this.pagina.url(),
                rqdata: this.dadosStripe.rqdata || null,
                chaveApi: chaveNopecha,
              });
            } else if (!chaveNopecha) {
              registrar('[CAPTCHA] NOPECHA_API_KEY não configurada — usando fallback de clique', 'warning');
            }

            // ── 4a. Injeta o token NopeCHA via postMessage do iframe + fetch verify_challenge ──
            if (tokenNopecha) {
              registrar('[CAPTCHA] Token NopeCHA recebido. Iniciando injeção multi-estratégia...', 'info');

              // ── (I) Diagnóstico + postMessage do iframe ───────────────────────────────
              const todosFramesHcaptcha = this.pagina.frames().filter(
                f => f.url().includes('newassets.hcaptcha.com'),
              );
              registrar(`[CAPTCHA] Frames hCaptcha encontrados: ${todosFramesHcaptcha.length}`, 'info');

              // Lê eKey, id de sessão e parentOrigin de DENTRO do iframe
              let eKeyGlobal = null;
              let parentOriginGlobal = null;
              let challengeIdGlobal = null;
              for (const frameHc of todosFramesHcaptcha) {
                try {
                  const info = await frameHc.evaluate(() => {
                    let eKey = null;
                    try { eKey = window.__HC_PARAMS__?.eKey || window.HC_PARAMS?.eKey || null; } catch (_) {}
                    try { if (!eKey) eKey = window.hcaptchaConfig?.eKey || null; } catch (_) {}
                    const hash = window.location.hash.replace(/^#/, '');
                    const ps = new URLSearchParams(hash);
                    if (!eKey) eKey = ps.get('eKey') || ps.get('element_key') || ps.get('sitekey') || null;
                    const challengeId = ps.get('id') || null;
                    let parentOrigin = null;
                    try { parentOrigin = document.referrer ? new URL(document.referrer).origin : null; } catch (_) {}
                    try { if (!parentOrigin) parentOrigin = window.parent.origin; } catch (_) {}
                    const fullUrl = window.location.href;
                    return { eKey, challengeId, parentOrigin, fullUrl };
                  });
                  registrar(`[CAPTCHA][DIAG] iframe info: eKey=${info.eKey} | id=${info.challengeId} | parentOrigin=${info.parentOrigin}`, 'info');
                  registrar(`[CAPTCHA][DIAG] iframe URL: ${info.fullUrl.substring(0, 200)}`, 'info');
                  if (info.eKey && !eKeyGlobal) eKeyGlobal = info.eKey;
                  if (info.challengeId && !challengeIdGlobal) challengeIdGlobal = info.challengeId;
                  if (info.parentOrigin && !parentOriginGlobal) parentOriginGlobal = info.parentOrigin;
                } catch (errDiag) {
                  registrar(`[CAPTCHA][DIAG] Erro lendo iframe: ${errDiag.message}`, 'warning');
                }
                break; // Diagnóstico no primeiro iframe
              }

              // Envia postMessage para todos os iframes hCaptcha
              let postMsgEnviado = 0;
              const targetOrigin = parentOriginGlobal || '*';
              for (const frameHc of todosFramesHcaptcha) {
                try {
                  await frameHc.evaluate(({ token, eKey, id, targetOrigin }) => {
                    const sendMsg = (data) => {
                      try { window.parent.postMessage(data, targetOrigin); } catch (_) {}
                      try { window.parent.postMessage(data, '*'); } catch (_) {}
                    };
                    // Formato 1 — objeto com eKey + id (formato mais completo)
                    sendMsg({ action: 'verifySuccess', token, eKey, id });
                    // Formato 2 — string JSON com eKey + id
                    sendMsg(JSON.stringify({ action: 'verifySuccess', token, eKey, id }));
                    // Formato 3 — sem id (como antes)
                    sendMsg({ action: 'verifySuccess', token, eKey });
                    sendMsg(JSON.stringify({ action: 'verifySuccess', token, eKey }));
                    // Formato 4 — challenge-complete
                    sendMsg({ event: 'e_challenge-complete', response: token, eKey, id });
                  }, { token: tokenNopecha, eKey: eKeyGlobal, id: challengeIdGlobal, targetOrigin });
                  postMsgEnviado++;
                } catch (errPm) {
                  registrar(`[CAPTCHA] Erro postMessage iframe: ${errPm.message}`, 'warning');
                }
              }
              if (postMsgEnviado > 0) {
                registrar(`[CAPTCHA] postMessage enviado em ${postMsgEnviado} iframe(s) (eKey=${eKeyGlobal || 'null'}, id=${challengeIdGlobal || 'null'}, target=${targetOrigin}). Aguardando 2s...`, 'info');
                await this.pagina.waitForTimeout(2000);
              }

              // ── (II) Chamada direta ao verify_challenge com client_secret ─────────────
              // O Stripe.js usa o client_secret do setup_intent para autenticar a chamada.
              // Capturamos o client_secret interceptando respostas da API (listener em inicializar()).
              const clientSecret = this.dadosStripe.clientSecretSI || '';
              if (verifyUrl && clientSecret) {
                try {
                  const resultadoVerify = await this.pagina.evaluate(async ({ token, url, clientSecret, chavePublica }) => {
                    const body = new URLSearchParams();
                    body.set('h-captcha-response', token);
                    body.set('client_secret', clientSecret);

                    const resp = await fetch(`https://api.stripe.com${url}`, {
                      method: 'POST',
                      headers: {
                    ...
                    return { status: resp.status, body: texto.substring(0, 800) };
                  }, {
                    token: tokenNopecha,
                    url: verifyUrl,
                    clientSecret,
                    chavePublica: this.dadosStripe.chavePublica || '',
                  });
                  registrar(`[CAPTCHA] verify_challenge direto (c/ client_secret): status=${resultadoVerify.status} | body=${resultadoVerify.body}`, 'info');
                } catch (errVc) {
                  registrar(`[CAPTCHA] Erro verify_challenge direto: ${errVc.message}`, 'warning');
                }
              } else if (verifyUrl && !clientSecret) {
                registrar('[CAPTCHA] verify_challenge direto: client_secret não capturado ainda — aguardando Stripe.js interceptar', 'warning');
              }

              // ── (III) Injeção DOM na página principal (complementar) ──────────────────
              try {
                await this.pagina.evaluate((token) => {
                  ...
                  if (window.hcaptcha && typeof window.hcaptcha.setResponse === 'function') {
                    try {
                      document.querySelectorAll('[data-hcaptcha-widget-id]').forEach((container) => {
                        const widgetId = container.getAttribute('data-hcaptcha-widget-id');
                ...
                registrar(`[CAPTCHA] Erro injeção DOM: ${errDom.message}`, 'warning');
              }

              registrar('[CAPTCHA] Token NopeCHA injetado (postMessage + verify_challenge + DOM). Aguardando Stripe.js processar...', 'success');

              // Print após injeção
              try {
                const dirPrints = path.resolve('screenshots');
                const ts = new Date().toISOString().replace(/[:.]/g, '-');
                await this.pagina.screenshot({ path: path.join(dirPrints, `captcha_depois_${ts}.png`), fullPage: true });
              } catch (_) {}
            } else {
              // ── 4b. Fallback: clica o #checkbox diretamente ────────────────────────
              registrar('[CAPTCHA] Fallback: clicando #checkbox diretamente...', 'info');
              ...
              if (frameHcaptcha) {
                try {
                  const checkbox = frameHcaptcha.locator('#checkbox');
                  await checkbox.waitFor({ state: 'visible', timeout: 15000 });
                  registrar('[CAPTCHA] Checkbox visível — clicando...', 'info');
                  await this.pagina.waitForTimeout(800);
                  await checkbox.click();
                  registrar('[CAPTCHA] Checkbox clicado!', 'success');
                } catch (errClick) {
                  registrar(`[CAPTCHA] Clique falhou: ${errClick.message}`, 'warning');
                  try {
                    const html = await frameHcaptcha.evaluate(() => document.body?.innerHTML?.substring(0, 500));
                    registrar(`[CAPTCHA] DOM do iframe: ${html}`, 'info');
                  } catch (_) {}
                }
              } else {
                registrar('[CAPTCHA] iframe não encontrado — sem fallback possível', 'warning');
              }
            ...
            // ── 5. Aguarda segundo /confirm OU redirect (até 120s) ─────────────────
            ...
            try {
              ...
              const printFinal = path.join(dirPrints, `captcha_final_${tsFinal}.png`);
              await this.pagina.screenshot({ path: printFinal, fullPage: true });
              registrar(`[CAPTCHA] Print final: ${printFinal}`, 'info');
            } catch (_) {}
            ...
            if (redirecionadoCaptcha || urlSucesso(this.pagina.url())) {
              ...
              return { sucesso: true, status: 'paid', mensagem: 'Pagamento confirmado (captcha resolvido)' };
            }
            ...
            // Sem segundo confirm nem redirect → captcha não resolveu (puzzle visual ou iframe não carregou)
            registrar('[CAPTCHA] Captcha não resolvido automaticamente após clique — puzzle visual exigido ou iframe não carregou', 'warning');
            return {
              sucesso: false,
              status: 'error',
              mensagem: 'Stripe exige verificação de captcha — checkbox clicado mas captcha exigiu puzzle visual',
            };
          }
